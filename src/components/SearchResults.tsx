...
const SearchResults: React.FC<Props> = ({
  ...
  useEffect(() => {
    ...
    return () => {
      {
        tab === '장소 보기' && activeDivision === '전체' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
      {
        tab === '장소 보기' && activeDivision === '음식점' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
      {
        tab === '장소 보기' && activeDivision === '문화시설' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
      {
        tab === '장소 보기' && activeDivision === '축제 공연 행사' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
      {
        tab === '장소 보기' && activeDivision === '관광지' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
      {
        tab === '장소 보기' && activeDivision === '레포츠' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
      {
        tab === '장소 보기' && activeDivision === '숙박' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
      {
        tab === '장소 보기' && activeDivision === '쇼핑' && (
          <div ref={placeLoadMoreRef}></div>
        );
      }
    };
  ...
  return (
    <div className="bg-white px-5 w-full h-full overflow-y-auto">
      {tab === '장소 보기' &&
        Object.keys(divisions).includes(activeDivision) &&
